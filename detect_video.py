...
from mask_temp.gate.PlaySound import Play_temperature, Play_Nomask
from mask_temp.gate.Arduino_serialCom import Temperature
# from servo import doorAutomate
# from utils.sev_tem import Temperature, doorAutomate
import time
# class detect:
def detect():
    #--------------- FLAGS --------------------------
    ...
    with open('mask_temp/gate/model/lbl.txt', 'r') as f:
        labels = [line.strip() for line in f.readlines()]

    print('[INFO] Loading model..')
    interpreter = tf.lite.Interpreter(model_path='mask_temp/gate/model/KAYA_detectMask.tflite',num_threads=4)  #experimental_delegates=[load_delegate('libedgetpu.so.1.0')])
